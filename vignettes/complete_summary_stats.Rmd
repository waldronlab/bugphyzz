...
## Number of genera per phylum per attribute in physiology dataset

```{r}
gen_phy_attr <- mapply(function(.x, .y) attribute_catalog(.x, .y), cat_rank, names(cat_rank), SIMPLIFY = FALSE)
```
```{r}
printGGlist(gen_phy_attr)
```
