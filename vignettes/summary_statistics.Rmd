...
Number of taxa in the `NCBI_ID` column across all of the datasets in bugphyzz:

```{r unique taxids in physiologies and fac}
physiologies_ncbi_ids <- lapply(physiologies, function(x) as.integer(x$NCBI_ID)) %>% 
  purrr::reduce(.x = ., .f = c) %>% 
  unique() %>% 
  .[!is.na(.)]

fac_ncbi_ids <- fac$NCBI_ID %>% 
  as.integer() %>% 
  unique() %>% 
  .[!is.na(.)]

ncbi_ids <- c(physiologies_ncbi_ids, fac_ncbi_ids) %>% 
  unique()
...
purrr::map_int(taxonomies, ~n_distinct(.x, na.rm = TRUE)) %>% 
  as_tibble(rownames = "taxonomy_rank") %>% 
  filter(taxonomy_rank != "NCBI_ID") %>% 
  mutate(taxonomy_rank = factor(taxonomy_rank, levels = taxonomy_rank)) %>% 
  ggplot(aes(taxonomy_rank, value)) +
  geom_col() +
...
gt_with_ranks %>% 
  ...
  geom_boxplot() +
  labs(title = "Growth temperature per phylum", y = "Growth temperature (°C)", 
       x = "Phylum") +
  theme_bw() +
...
```{r}
n_attributes <- purrr::map_int(physiologies, function(x) length(unique(x$Attribute))) %>% 
  as_tibble(rownames = "rownames") %>% 
  magrittr::set_colnames(c("dataset", "n_attributes")) %>% 
