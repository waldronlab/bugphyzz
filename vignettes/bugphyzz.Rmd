---
title: "Data curation for bugphyzz"
...
If using Google Sheets, publish a csv file to web:
 4. Add Google Doc "publish to web" link and add in Links (**link to file in GitHub**)
 5. Add Google Doc link to Source Link (**link to file in GitHub**)
...
map_depth(.x = aer_sig, .depth = 2, .f = ~ length(.x)) |> 
  unlist()
map_depth(.x = aer_sig, .depth = 2, .f = ~ head(.x))
```
...
map_depth(.x = acid_sig, .depth = 2, .f = ~ length(.x)) |> 
  unlist()
map_depth(.x = acid_sig, .depth = 2, .f = ~ head(.x))
```
