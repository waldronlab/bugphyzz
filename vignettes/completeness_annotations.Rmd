---
title: "Completeness of annotations based on NCBI IDs"
output:
  rmarkdown::html_vignette:
      toc: yes
vignette: >
  %\VignetteIndexEntry{Completeness of annotations based on NCBI IDs}
...
This document describes the completeness of taxonomy rank annotations in all
BugPhyzz datasets, i.e the percentage of taxonomy ranks available in BugPhyzz.

Sections:

+ **"All taxonomies".** Describes the code to get the ranks (and full taxonomy
classification) of all unique NCBI IDs in BugPhyzz, and
how they were merged with the bugphyzz datasets.

+ **"Completeness of datasets across bugphyzz".** Describes the overall
completeness of each BugPhyzz dataset in a single plot.

+ **`attributeSummaryByRank` function.** Describes a function to create a
summary table of completeness per attribute.

+ More detailed completeness information for each attribute is described in the
form of tables in the **"Categorical datasets in physiologies"**, 
**"Numeric attributes in physiologies”**, **"Other datasets"**, and
**"Fatty acid composition"**, as the result of applying the `attributeSummaryByRank`
function.
...
ncbi_ids_physiologies <- unlist(lapply(physiologies, function(x) as.numeric(x$NCBI_ID)))
ncbi_ids_fac <- as.numeric(fac$NCBI_ID)
...
# Completeness of datasets across bugphyzz
...
data %>% 
  ...
  labs(x = "Dataset", y = "NCBI_ID",
       title = "Completeness of Taxa Annotations in Bugphyzz",
...
**Figure. Completeness of taxa annotations in BugPhyzz datasets.** Taxonomy rank
annotations were based on the NCBI taxonomy, i.e. NCBI IDs. The color legend
represents the ranks of the NCBI IDs (not considering inherited ranks);
"NA" refers to NCBI IDs missing or not available,
and "no rank" refers to NCBI IDs that don't have a formal taxonomy rank in the
NCBI database (more info at https://www.ncbi.nlm.nih.gov/pmc/articles/PMC7408187/).


# `attributeSummaryByRank` function

+ This function (`attributeSummaryByRank`) takes a dataset from bugphyzz (with
merged taxonomy)
...
+ A `no_rank` in the column names indicate organisms not formally classified
