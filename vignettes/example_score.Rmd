...
Import attribute values of cellular respiration ("aerophilicity" data set).

```{r}
aerophilicity <- physiologies(keyword = "aerophilicity")
aerophilicity <- tibble::as_tibble(aerophilicity[[1]])
aerophilicity[ , c("NCBI_ID", "Attribute_value")] %>% 
  head() %>% 
  knitr::kable()
```
...
Sum OTUs by genus
```{r}
v13summed <- assay(v13dat) %>%
  as_tibble() %>%
  filter(!is.na(rowData(v13dat)$GENUS)) %>%
  group_by(rowData(v13genus)$GENUS) %>%
  summarise_all(sum)
v13genus <- 
dim(v13summed)
```
